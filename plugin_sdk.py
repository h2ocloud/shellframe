...
class PluginRegistry:
    ...
    def dispatch_action(self, plugin_name: str, action: str, args: dict) -> dict:
        for p in self.plugins:
            if p.manifest.name != plugin_name:
                continue
            handler = getattr(p, "action", None)
            if not callable(handler):
                return {"ok": False, "message": f"{plugin_name} has no action handler"}
            res = self._safe(handler, action, args)
            if isinstance(res, dict):
                return res
            return {"ok": True, "result": res}
        return {"ok": False, "message": f"plugin not found: {plugin_name}"}
    ...
    def collect_settings_panels(self) -> list[dict]:
        ...
        for p in self.plugins:
            ...
            sidebar_js = self._safe(p.asset, "sidebar.js") or ""
            styles = self._safe(p.asset, "styles.css") or ""
            out.append({
