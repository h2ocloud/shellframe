...
class Api:
    ...
    def __init__(self):
        ...
        # ── Plugin system (best-effort, never blocks startup) ──
        self._plugins = None
        try:
            import plugin_sdk
            ...
            _dlog("plugins", f"registry online with {len(self._plugins.plugins)} plugin(s): "
                              f"{[p.manifest.name for p in self._plugins.plugins]}")
        except Exception as _e:
            _dlog("plugins", f"init skipped: {_e!r}")
            try:
                import traceback
                _dlog("plugins", traceback.format_exc())
            except Exception:
                pass
    ...
    # ── Plugin bridge (read-only — JS asks, plugins answer) ──

    def list_plugin_panels(self) -> str:
        """Returns array of plugin metadata + injected HTML/CSS/JS for the
        Settings overlay. The frontend renders each entry as a tab."""
    ...
    def plugin_sidebar_badges(self, sid: str) -> str:
        """Return concatenated HTML of all plugin badges for one session row."""
        if not self._plugins:
            return ""
        try:
