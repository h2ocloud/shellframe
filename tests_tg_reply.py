...
# total<30 = 原 _extract_marked_mobile_reply（等待串流），
# total>=30 = 原 _extract_marked_mobile_reply_force（取最後完整）。
def extract(slot):
...
ok("streaming: normal waits", extract(mkslot(raw3)) == "")
ok("streaming: force last complete",
   extract_force(mkslot(raw3)) == "舊的完整回應")
